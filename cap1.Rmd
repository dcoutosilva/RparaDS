...
```
Podemos configurar as propriedades manualmente deixando todos os pontos azuis por exemplo
...
As variáveis ficam isoladas, cada gráfico para cada variável, ao inves de um grafíco para muitas variaveis.

Exercício 5
nrow dividi-se em linhas
ncol dividi-se em colunas
facet_grid não tem variaveis nrow e ncol pois são de acordo com variaveis as quais você insere de acordo
...
duplicação de código, pois estou plotando os mesmo dados em dois tipos de gráficos diferentes péssimo para manutenção do código.. o mais correto seria.
...
para limpeza do código, é o mais correto..podemos modificar umas das propriedades
```{r}
ggplot(
  data = mpg, mapping = aes(
    x = displ, y = hwy
...
dodge coloca objetos sobrepostos diretamente um ao lado do outro, facilita a comparação de valores individuais
```{r}
ggplot(
  data = mpg
) +
...
espalha os pontos, porque não é provável que dois pontos quaisquer receba a mesma quantidade de ruídos aleatório,
```{r}
ggplot(
  data = mpg
) + geom_point(
...
coord_polar usa coordenadas polares, que revelam uma conexão interessante entre um gráfico de barras e gráfico de setores.
```{r}
bar
```
...
exer 3 
coord_quickmap X coord_map ---  se pegarmos o mapa da NZ.
```{r}
...
coord_map utiliza-se mapas de projeção de 3 dimensões.. projeção de mercator

coord_quickmap é a plotagem mais rapida, porem ela ignora a curvatura da terra
